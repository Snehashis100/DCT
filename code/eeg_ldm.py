...
def create_trainer(num_epoch, precision=32, accumulate_grad_batches=2,logger=None,check_val_every_n_epoch=0):
    ...
    # filename="latest_checkpoint.ckpt",
    # save_top_k=1,
    # mode="max",
    # monitor="current_epoch",  # or "global_step"
    # )
    return pl.Trainer(accelerator=acc, max_epochs=num_epoch, logger=logger, 
            ...
            check_val_every_n_epoch=check_val_every_n_epoch, devices='auto')
...
if __name__ == '__main__':
    ...
    config.output_path = config.output_path.split(r"\results")[0]
