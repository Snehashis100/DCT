...
def accuracy(pred, target, top_k=1):
    """
    Computes top-k accuracy for a classification task.

    Args:
        pred (torch.Tensor): The prediction tensor of shape [batch_size, num_classes].
        target (torch.Tensor): The target tensor of shape [batch_size].
        top_k (int): The top-k value to compute the accuracy.

    Returns:
        float: The top-k accuracy.
    """
    with torch.no_grad():
        max_k = min(top_k, pred.size(1))
        _, pred_k = pred.topk(max_k, 1, True, True)  # Get top-k predictions
        pred_k = pred_k.t()  # Transpose to shape [top_k, batch_size]
        target = target.view(1, -1).expand_as(pred_k)  # Expand target shape
        
        correct = pred_k.eq(target)
        correct_k = correct[:max_k].view(-1).float().sum(0, keepdim=True)  # Sum of correct predictions
        
        return correct_k.mul_(100.0 / pred.size(0))  # Return accuracy as percentage
...
def n_way_top_k_acc(pred, class_id, n_way, num_trials=40, top_k=1):
    ...
    for t in range(num_trials):
        ...
        # print(pred_picked.shape)
        # print(pred_picked)
        # print("+++++++++++++++++++n_way_top_k_acc +++++++++++++++++++++++")
        # print("pred_picked.unsqueeze(0)",pred_picked.unsqueeze(0))
        # print("pred_picked.unsqueeze(0)",pred_picked.shape)
        # print("pred_picked.squeeze(0)",pred_picked.squeeze(0))
        acc = accuracy(pred_picked.unsqueeze(0), torch.tensor([0], device=pred.device), # .flatten()
                    top_k=top_k)#, task='multiclass', num_classes=1000)
