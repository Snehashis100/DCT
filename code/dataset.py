...
class EEGDataset(Dataset):
    ...
    def __init__(self, eeg_signals_path, image_transform=identity, subject = 4):
        ...
        self.labels = loaded["labels"]
        self.images = loaded["images"]
        self.classMap = {'n02106662': 'German shepherd', 'n02124075': 'Egyptian cat', 'n02281787': 'Lycaenid butterfly',
                        'n02389026': 'Sorrel horse', 'n02492035': 'Capuchin', 'n02504458': 'Elephant',
                        'n02510455': 'Panda', 'n02607072': 'Anemone fish', 'n02690373': 'Airliner', 
                         'n02906734': 'Broom', 'n02951358': 'Canoe', 'n02992529': 'Cellphone',
                        'n03063599': 'Mug', 'n03100240': 'Convertible', 'n03180011': 'Desktop PC', 
                        'n03197337': 'Digital watch', 'n03272010': 'Electric guitar', 'n03272562': 'Electric locomotive',
                        'n03297495': 'Espresso maker', 'n03376595': 'Folding chair', 'n03445777': 'Golf ball', 
                        'n03452741': 'Piano', 'n03584829': 'Iron', 'n03590841': 'Jack-o’-lantern', 
                        'n03709823': 'Mailbag', 'n03773504': 'Missile', 'n03775071': 'Mitten', 
                        'n03792782': 'Mountain bike', 'n03792972': 'Mountain tent', 'n03877472': 'Pyjama', 
                        'n03888257': 'Parachute', 'n03982430': 'Pool table', 'n04044716': 'Radio telescope', 
                        'n04069434': 'Reflex camera', 'n04086273': 'Revolver', 'n04120489': 'Running shoe', 
                         'n07753592': 'Banana', 'n07873807': 'Pizza', 'n11939491': 'Daisy', 'n13054560': 'Bolete'
                        }
        self.imagenet = r'..\DreamDiffusion\datasets\imageNet_images\imageNet_images'
        
        self.image_prompt_dict = {}
        with open(r'..\DreamDiffusion\datasets\imageNet_images\imageNet_descriptions.txt') as f:
            prompts = [line.rstrip('\n') for line in f]
            f.close()
        ...
        for i in range(len(prompts)//2):
            ind1 = 2*i
            ind2 = (2*i) +1
            img_name = prompts[ind1].split(' :')[0]
            text = prompts[ind2].split('[/INST] ')[1]
            if img_name not in self.image_prompt_dict.keys():
                self.image_prompt_dict[img_name] = text
            else:
                raise ValueError("Error occured as the current key is already present in the image prompt dict")
    ...
    def __getitem__(self, i):
        ...
        try:
            ...
            print(f"++++++++++++++++{image_path} doesnot exists+++++++++++++++")
        ...
        # image_name = self.images[self.data[i]["image"]]
        # image_path = os.path.join(self.imagenet, image_name.split('_')[0], image_name+'.JPEG')
        class_fname = image_name.split('_')[0]
        if class_fname in self.classMap.keys():
            class_name = self.classMap[class_fname]
        else:
            raise ValueError(f'\'{class_fname}\' is not present as a key in the class mapping dict')

        img_name = image_name+'.JPEG'
        if img_name in self.image_prompt_dict.keys():
            img_desc = self.image_prompt_dict[img_name]
        else:
            raise ValueError(f'\'{img_name} is not present in the image prompt dict')

        # image_raw = Image.open(image_path).convert('RGB') 
        image = np.array(image_raw) / 255.0
        image_raw = self.processor(images=image_raw, return_tensors="pt")
        # text_raw = self.processor(text=img_desc, return_tensors="pt") # processor(text=x, padding='max_length', max_length=77, truncation=True)
        ...
        # print(label.shape)
        # print(image.shape)
        # print(image_raw['pixel_values'].shape)
        # print(img_desc)
        # print(text_raw['input_ids'].shape)
        # print(class_name)
        
        return {'eeg': eeg, 'label': label, 'image': image1, 
                'image_raw': image_raw, 'prompt': img_desc, 'class_name': class_name} #'prompt_raw': text_raw,
