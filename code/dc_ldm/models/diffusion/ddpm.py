...
from dc_ldm.modules.encoders.modules import FrozenImageEmbedder, FrozenCLIPEmbedder
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        if self.learn_logvar:
            self.logvar = nn.Parameter(self.logvar, requires_grad=True).to('cuda')
    ...
    def get_input(self, batch, k):
        x = batch[k]
        # print("++++++ inside get_input func of DDPM++++++++++")
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        if self.clip_tune:
            ...
            self.text_embedder = FrozenCLIPEmbedder()
        self.cls_tune = False
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if bs is not None:
            x = x[:bs]
        x = x.to(self.device)
        # print("+++++++++ inside get_input() of LD class+++++++++++++")
        # print("k: ",k)
        # print("x: ",x.shape)
        encoder_posterior = self.encode_first_stage(x)
        ...
        out = [z, c , batch['label'], batch['image_raw'], batch["prompt"], batch["class_name"]]
    ...
    def shared_step(self, batch, **kwargs):
        ...
        x, c, label, image_raw, prompt, classname = self.get_input(batch, self.first_stage_key)
        ...
        # print(c.shape)
        if self.return_cond:
            loss, cc = self(x, c, label, image_raw, prompt, classname)
            return loss, cc
        else:    
            loss = self(x, c, label, image_raw, prompt, classname)
            return loss

    def forward(self, x, c, label, image_raw, prompt, classname, *args, **kwargs):
        # print(self.num_timesteps)
        # print("x: ",x.shape)
        # print("c: ",c.shape)
        # print("label: ",label.shape)
        # print("image_raw: ",image_raw['pixel_values'].shape)
        # print("prompt: ",prompt)
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                ...
                # print("++++++++++ inside forward of LD++++++++++++++++")
                # print('c.shape')
                # print(c.shape) # torch.Size([4, 77, 768])
                # print('re_latent.shape') # torch.Size([4, 128, 1024])
                # print(re_latent.shape)
                # print('x: ',x)
                # print('c: ',c)
                # print('t: ',t)
        ...
        if self.clip_tune:
            ...
            text_embeds = self.text_embedder(prompt)
            # print('re_latent.shape') # torch.Size([4, 128, 1024])
            # print(re_latent.shape)
            # print("++++++++++++++++++++++++++++++")
            # print('image_embeds.shape') # torch.Size([4, 768])
            # print(image_embeds.shape)
            # print("++++++++++++++++++++++++++++++")
            # print('text_embeds.shape') # torch.Size([4, 768])
            # print(text_embeds.shape)

            
            loss_clip_img = self.cond_stage_model.get_clip_loss(re_latent, image_embeds)
            loss_clip_text = self.cond_stage_model.get_clip_loss(re_latent, text_embeds)

            loss_clip = loss_clip_img + loss_clip_text
